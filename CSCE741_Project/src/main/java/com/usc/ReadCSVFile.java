...
public class ReadCSVFile {
	...
	public List<String[]> getCoursesByWhere(String where, String value) throws IOException {
		...
		for (String next, line = br.readLine(); line != null; line = next) {
			...
			// if the next line is related to current section: this is the only
			// special case found in CSV file.
			// There is no occurrence more than twice for each section.
			if (next != null) {
				String[] p2 = next.replaceAll("\\s+(?=[,])", "").split(",");
				// if the next line related to current one. In this file case,
				// if there is no value, it means there is other info. for this
				// section needs to be extracted.
				if (p2[0].equals("")) {
